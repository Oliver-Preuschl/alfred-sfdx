...
const getDefaultDevhubUsername = async (projectPath) => {
  ...
  try {
    const sfdxPropertyLines = await getSfdxPropertyLines(
      `cd "${process.env.workspace}/${projectPath}"; sfdx config:get defaultdevhubusername`,
      2
    );
    const key2Value = getKeyValueMap(sfdxPropertyLines, "Name", "Value");
    defaultDevhubUsername = key2Value.has("defaultdevhubusername")
      ? key2Value.get("defaultdevhubusername")
      : "";
  } catch (e) {}
